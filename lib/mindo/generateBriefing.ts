...
function templateFallback(facts: DailyFacts): string {
  if (facts.todaysAssignedMissions.length > 0) {
    return `Good morning. Here's a suggestion for today: try "${facts.todaysAssignedMissions[0].title}" when you have a few minutes.`;
  }
  if (facts.last7Days.currentStreak > 0) {
    return `Good morning. You're keeping up a nice streak — take a moment today to check in with how you're feeling.`;
  }
  return `Good morning. Take a moment today to check in with how you're feeling.`;
}
